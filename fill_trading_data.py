...
def cudf_fill_analysis_data_v1(original_data):	
	analysis_dates = pd.to_datetime(original_data.loc[:, "datetime"]).dt.date.unique().tolist()
	...
	for date_index in range(len(analysis_dates)):
		current_date = np.datetime64(analysis_dates[date_index])
		dates_ticker_data = (original_data.loc[original_data['datetime']>=current_date])
		dates_ticker_data = (dates_ticker_data.loc[dates_ticker_data['datetime']< current_date+np.timedelta64(1, 'D')]).reset_index(drop=True)

		initial_rows = cudf_fill_initial_rows(dates_ticker_data)
		middle_rows = cudf_fill_middle_rows(dates_ticker_data)
		final_rows = cudf_fill_final_rows(dates_ticker_data)
...
def cudf_fill_trade_training_data_v1(original_data):
	trade_day_training_data = pd.DataFrame(columns=["datetime", "open", "high", "low", "close", "volume"])
	
	days_date = pd.to_datetime(original_data.loc[:, "datetime"]).dt.date.unique().tolist()

	initial_rows_DF = cudf_fill_initial_rows(original_data)
	filled_middle_data_DF = cudf_fill_middle_rows(original_data)
	final_rows_DF = cudf_fill_final_rows(original_data)	
...
def cudf_fill_initial_rows(original_data):
	datas_date = pd.to_datetime(original_data.loc[:, "datetime"]).dt.date.tolist()[0]
	start_time = dt.time(9,30,0)
	const_start_time = np.datetime64(dt.datetime.combine(datas_date, start_time)).astype('datetime64[ns]')

	initial_rows_DF = pd.DataFrame(columns=["open", "high", "low", "close", "volume"])

	orig_data_start_time = original_data.loc[0, "datetime"]
	orig_data_init_open = original_data.loc[0, "open"]
	orig_data_init_high = original_data.loc[0, "high"]
	orig_data_init_low = original_data.loc[0, "low"]
	orig_data_init_close = original_data.loc[0, "close"]

	if(orig_data_start_time > const_start_time):
		# input("Data Start Time is not 9:30:00")
		initial_time_DF = pd.DataFrame(columns=["datetime"])
		df_row_counter = 0
		orig_data_init_volume = 0

		curr_start_time = const_start_time
		orig_data_init_row = {"open":orig_data_init_open, "high":orig_data_init_high, "low":orig_data_init_low, "close":orig_data_init_close, "volume":orig_data_init_volume}

		while(curr_start_time != orig_data_start_time):
			initial_time_DF = initial_time_DF.append({"datetime":curr_start_time}, ignore_index=True)
			initial_rows_DF = initial_rows_DF.append(orig_data_init_row, ignore_index=True)

			df_row_counter += 1
			curr_start_time = curr_start_time + np.timedelta64(1, 'm')
...
def cudf_fill_final_rows(original_data):
	datas_date = pd.to_datetime(original_data.loc[:, "datetime"]).dt.date.tolist()[0]
	end_time = dt.time(16,0,0)
	const_end_time = np.datetime64(dt.datetime.combine(datas_date, end_time)).astype('datetime64[ns]')

	final_rows_DF = pd.DataFrame(columns=["open", "high", "low", "close", "volume"])

	orig_data_end_time = original_data.datetime.iloc[-1]
	orig_data_final_open = original_data.open.iloc[-1]
	orig_data_final_high = original_data.high.iloc[-1]
	orig_data_final_low = original_data.low.iloc[-1]
	orig_data_final_close = original_data.close.iloc[-1]	
	...
	if(orig_data_end_time < const_end_time):
		#print("Data End Time is not 16:00:00")
		final_time_DF = pd.DataFrame(columns=["datetime"])
		df_row_counter = 0
		orig_data_final_volume = 0

		curr_end_time = orig_data_end_time + np.timedelta64(1, 'm')
		orig_data_final_row = {"open":orig_data_final_open, "high":orig_data_final_high, "low":orig_data_final_low, "close":orig_data_final_close, "volume":orig_data_final_volume}
		
		while(curr_end_time != const_end_time):
			final_time_DF = final_time_DF.append({"datetime":curr_end_time}, ignore_index=True)
			final_rows_DF = final_rows_DF.append(orig_data_final_row, ignore_index=True)

			df_row_counter += 1
			curr_end_time = curr_end_time + np.timedelta64(1, 'm')
...
def cudf_fill_middle_rows(original_data):
	days_date = original_data.loc[:, "datetime"].dt.date.tolist()[0]

	filled_data_DF = pd.DataFrame(columns=["datetime", "open", "high", "low", "close", "volume"])
	
	for row_index in range(len(original_data)):
		curr_data_row = original_data.loc[[row_index]]
		curr_data_row_datetime = curr_data_row.loc[row_index, "datetime"]

		if(row_index != len(original_data)-1):
			next_data_row = original_data.loc[[row_index+1]]
			next_data_row_datetime = next_data_row.loc[row_index+1, "datetime"]

		time_difference = int((next_data_row_datetime - curr_data_row_datetime).seconds/60)

		proper_next_data_row_datetime = curr_data_row_datetime + np.timedelta64(1, 'm')
		...
		if(proper_next_data_row_datetime != next_data_row_datetime):
			...
			for minutes in range(time_difference-1):
				if(row_index < len(original_data)-1):
					prop_next_datetime = (proper_next_data_row_datetime + np.timedelta64(minutes, 'm'))
					prop_next_open = curr_data_row.open.iloc[0]
					prop_next_high = curr_data_row.high.iloc[0]
					prop_next_low = curr_data_row.low.iloc[0]
					prop_next_close = curr_data_row.close.iloc[0]
					...
					prop_next_row = {"datetime":prop_next_datetime, "open":prop_next_open, "high":prop_next_high, "low":prop_next_low, "close":prop_next_close, "volume":prop_next_volume}
					filled_data_DF = filled_data_DF.append(prop_next_row, ignore_index = True)
